...
class LikeDetailViewTests(APITestCase):
    def setUp(self):
        ...
        angelo = User.objects.create_user(
            username='angelo', password='password')
        Post.objects.create(
            owner=samuel, title='wildlife',
            description='spring', country='Lebanon'
        )
        Post.objects.create(
            owner=angelo, title='seaside',
            description='summer', country='Greece'
        )
